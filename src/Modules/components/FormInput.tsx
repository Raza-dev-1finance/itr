      ...
      {formData.map((data, index) => (
        <div key={index} className="flex flex-col pb-[20px] w-full" >
          <label className='text-[#0A0A0A] self-start font-["Fira Sans"] text-[14px] font-[400] leading-8 tracking-[0.4px]'>
            {data.label}{!data?.areatext && <sup className='text-red-600'>*</sup>}
          </label>
          <div className="w-full flex flex-row justify-center pt-[2px] relative">
            {data.options?<>
              <div onClick={()=>onClick && onClick(data.name)} ref={wrapperRef}  key={data.name} className={cn(` flex flex-row justify-between w-full p-[16px] flex-1 rounded-[4px] cursor-pointer
                appearance-none border border-[#A3A3A3] focus:ring-0 focus:ring-offset-0
                focus:border-[#A3A3A3] bg-white text-[16px] font-[400] leading-[16px] text-[#000] font-['Fira Sans']`)}>{data.placeholder}
            ...
            {data.name==="state" && statearr && (<div  ref={wrapperRef} style={{ boxShadow: '0px 2px 8px 0px rgba(0, 0, 0, 0.02)' }} className='w-[349px] h-[264px] absolute top-[55px] border-[1px] border-[#61625E] bg-white rounded-b-[2px] z-30'>
                <div  className='flex py-[12px] px-[16px] cursor-pointer text-[#171717] font-["Fira Sans"] text-[14px] font-normal leading-5' onClick={() => { setvalue((prev) => ({ ...prev, state: 'UP' }))}}>Uttar Prasdesh</div>
            </div>)}
          ...
          {/* {errorModal && (
            <div className="w-full flex flex-row justify-start pt-[2px] relative ">
              <div className="text-red-500 text-sm">Please enter a valid PAN number.</div>
            </div>
          )} */}
        </div>
      ))}
